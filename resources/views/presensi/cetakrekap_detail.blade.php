...
<body class="A4 landscape">
    ...
    <section class="sheet padding-10mm">
      ...
      for($i=1; $i <= $jmlhari; $i++){
          ...
          $ceklibur = cekkaryawanlibur($datalibur, $search_items);//cari karyawan libur
          ...
          if($r->$tgl != NULL){
              ...
              $jam_terlambat = $j_terlambat[0];
                ...
                $total_jam_kerja = hitungjamkerja($tgl_presensi,$jam_mulai,$jam_out,$total_jam,$lintashari, $jam_awal_istirahat,$jam_akhir_istirahat,$terlambat );
              }else{
                $total_jam_kerja = 0;
              }
        ...
        ?>
                    <td style="background-color: {{ $color }}">
                        @if ($status == 'h')
                            <span style="font-weight: bold">{{ $nama_jam_kerja }}</span>
                            <br>
                            <span style="color:rgb(72, 135, 245)">({{ $jam_masuk }} - {{ $jam_pulang }})</span> 
                            <br>
                            <span style ="color: orange">{{ $jam_in }} - {{ $jam_out }}</span>
                            <br>
                            <span style ="color: rgb(12, 97, 21)">Total Jam:{{ $total_jam_kerja}}</span>
                            <br>
                            @if ($terlambat_desimal > 0)
                            <span style ="color: red">Terlambat:{{ $terlambat }} ( {{ $terlambat_desimal }} Jam )</span> 
                            <br>
                            Denda : {{ $denda }}
                            @endif
